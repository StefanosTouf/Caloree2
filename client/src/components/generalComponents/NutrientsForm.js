...
const AddCustomFood = ({ handleSubmit, onSubmit, label, foodDescription }) => {
  ...
  const onHandleSubmit = (formValues) => {
    const { description, ...others } = formValues;
    onSubmit({ description, nutrients: others });
  };
    ...
    <>
      ...
      >
        {foodDescription ? (
          <Field
            name="description"
            component={(props) => renderInput(props, 'text')}
            label="Description"
          />
        ) : (
          <></>
        ...
        <button className="ui button primary" style={{ margin: '1rem' }}>
          Submit
        </button>
      </form>
    </>
