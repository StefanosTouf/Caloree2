...
const GeneralInfo = ({
  ...
  const renderInfo = () => {
    const { energyAch, proteinAch, carbAch, fatAch, fiberAch } = _.mapKeys(
      targetsAchieved,
      (_value, key) => {
        return key + 'Ach';
      }
    ...
    const { energyTarg, proteinTarg, carbTarg, fatTarg, fiberTarg } = _.mapKeys(
      userTargets,
      (_value, key) => {
        return key + 'Targ';
      }
        ...
        <div className="ui olive inverted progress">
          ...
          <div className="bar-wrapper">
            <div
              className="bar"
              style={{
                width: `${(energyAch.amount / energyTarg.amount) * 100}%`,
              }}
            />
          </div>
        </div>
        <div className="ui green inverted progress">
          ...
          <div className="bar-wrapper">
            <div
              className="bar"
              style={{
                width: `${(proteinAch.amount / proteinTarg.amount) * 100}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="ui brown inverted progress">
          ...
          <div className="bar-wrapper">
            <div
              className="bar"
              style={{
                width: `${(fiberAch.amount / fiberTarg.amount) * 100}%`,
              }}
            />
          </div>
        </div>
...
export default connect(mapStateToProps, { fetchLog, fetchUser })(GeneralInfo);
